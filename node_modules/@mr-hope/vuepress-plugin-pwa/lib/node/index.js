...
const pwaPlugin = (options, context) => {
    ...
    pwaI18nConfig["/"] = pwaI18nConfig[(0, vuepress_shared_1.getRootLangPath)(context)];
    const config = {
        name: "pwa",
        define: () => ({
        ...
        beforeDevServer(app) {
            app.get(`${base || "/"}manifest.webmanifest`, (_req, res) => {
                (0, genManifest_1.getManifest)(pwaOption, context)
                    .then((manifest) => {
                    res.send(manifest);
                })
        ...
        ready() {
            context.siteConfig.head = (0, injectHead_1.injectLinkstoHead)(pwaOption, base, context.siteConfig.head);
        },
        async generated() {
            await (0, genManifest_1.genManifest)(pwaOption, context);
            await (0, genServiceWorker_1.genServiceWorker)(pwaOption, context);
        },
    };
